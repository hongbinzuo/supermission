...
- 主栈采用 Bun + TypeScript。
- `work.yaml` 和 task ledger 使用 `yaml` 解析。
...
- `supermission run` 目前只是记录外部 agent 或人工实现动作，不调用真实模型。
- `supermission validate` 会真实执行 shell 命令，并保存 stdout/stderr。
- 缺少 validation commands 时会写入 `validation_missing` supervisor signal。
- `supermission inspect` 可以按 zero-based index 或稳定 record id 查看单条 `events`、`telemetry`、`tool-calls` 或 `supervisor` 记录。
- `supermission handoff` 在 mission 已 `validated` 时会把状态推进到 `completed`。
...
- `supermission change propose` 会创建结构化 change proposal，并将 mission 状态推进到 `needs_decision`。
- `supermission change approve/reject` 会写入 decision event，并恢复 proposal 前的 mission 状态。
- `supermission change apply` 已支持在 change 批准后安全追加 acceptance、validation commands、workflow steps 到 `work.yaml`，并可追加受控 plan notes 到 `plan.md`。
- apply 会记录 `change.applied` event、change application metadata 和 `decisions.md` 证据；更复杂的结构化 `plan.md` patch/apply 机制仍待后续实现。
- `supermission diff` 会把当前 git diff 捕获到 `patch.diff`，默认排除 `.supermission/**` 以避免 mission evidence 污染 patch snapshot。
- patch snapshot 会同时捕获 tracked changes 和 untracked files。
- `supermission diff --task <task-id>` 会按 task scope 捕获 patch，同时对当前所有 git changes 执行 scope audit，越界时仍写入 `scope_drift` evidence。
- `supermission checkpoint create/list` 支持非破坏性 checkpoint capture，默认排除 `.supermission/**`；`checkpoint create --task <task-id>` 支持按 task scope 捕获 checkpoint patch；自动 rollback 仍是 TBD。
- `supermission branch create` 会创建 supermission branch，但不切换当前分支。
- `supermission worktree create` 必须显式传入 `--path`，避免工具偷偷创建位置不明的工作区。
- `supermission rollback-plan` 只生成人工可审查的回退计划，不执行回滚。
- `supermission rollback-check` 会用 checkpoint patch 执行非破坏性的 reverse apply check，确认当前工作区是否能干净回退；失败时写入 `merge_conflict` supervisor signal。
- `supermission doctor` 会诊断 mission 健康度，发现 blocking 问题时返回非零 exit code。
- `supermission monitor` 会生成 `monitor.md`，并展示 active tasks、ready tasks、blocked tasks、pending changes、supervisor signals、recent events 和 next actions。
- validation command 多次重复失败时会写入 `repeated_failure` supervisor signal，并由 `supermission doctor` / `supermission monitor` 展示为 blocking finding。
...
- `supermission validate` 默认阻止明显危险命令，例如 `rm -rf`、`git reset --hard`、`sudo`、`docker system prune`、`kubectl delete` 等；必须先通过 `approve_risky_command` gate，并显式传 `--allow-risky` 才会执行。
- 可选 `.supermission/policy.yaml` 支持 `validation_allowlist`；存在 allowlist 时，不匹配的 validation command 会被阻止并写入 `command_policy_blocked` supervisor signal。
- 可选 `.supermission/policy.yaml` `redaction.patterns` 可为验证输出追加自定义正则脱敏规则。
- `supermission policy init/show` 支持创建和查看项目 policy 文件。
...
- supermission monitor report and supervisor signal inspection
...
- supermission doctor health checks
...
- `supermission run` 后当前进入 `needs_review`。
...
- patch snapshot 已默认排除 `.supermission/**`，已覆盖 tracked/untracked files，并支持按 task scope 捕获；后续可按更完整 mission scope 捕获。
